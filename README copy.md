...
`.env.example` is an exact, ready-to-use copy of the local `.env` configuration.
For a new checkout:
...
host requires it. The root
`.env` is the canonical active configuration in both modes: online setup copies
`.env.example`, while the offline installer generates `.env` from
`config/offline.env.template` with target paths and secrets.
